...
export function BuildingSummaryCards({
  actualConcreteTotal,
  ...
  completedFloorsCount: number;
  estimatedConcreteTotal: number;
  inProgressFloorsCount: number;
  remainingConcrete: number;
  totalFloors: number;
}) {
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
      <SummaryCard label="Total Floors" value={String(totalFloors)} />
      ...
      <SummaryCard label="Completed Floors" value={String(completedFloorsCount)} />
      <SummaryCard label="In Progress Floors" value={String(inProgressFloorsCount)} />
    </div>
  );
}
